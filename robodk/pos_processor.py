...
# Originally, this post was named Staubli_VAL3_InlineMove
#
...
PROGRAM_PJX_CS8V7 = '''<?xml version="1.0" encoding="utf-8"?>
<Project xmlns="http://www.staubli.com/robotics/VAL3/Project/3">
  <Parameters version="s7.11.2" stackSize="5000" millimeterUnit="true" />
<Programs>
<Program file="MoveJoint.pgx" />
<Program file="MoveLine.pgx" />
%s<Program file="start.pgx" />
...
# Example: Use post Staubli_S6 instead
PROGRAM_PJX_CS8V6 = '''<?xml version="1.0" encoding="utf-8"?>
<project xmlns="ProjectNameSpace">
  <parameters version="s6.9.1" stackSize="5000" millimeterUnit="true" />
  <programSection>
  ...
  <aliasSection>
    ...
    <alias name="vg" interface="Disk://vg" autoload="true" />
    <alias name="rp" interface="Disk://appli" autoload="true" />
    <alias name="us" interface="Disk://usinage" autoload="true" />
  </aliasSection>
</project>
'''
...
<Database xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://www.staubli.com/robotics/VAL3/Data/2">
  <Datas>
    ...
    <Data name="pPointRx" access="private" xsi:type="array" type="pointRs" size="1">
      <Value key="0" fatherId="fFrame1[0]" />
    </Data>
    <Data name="pPointRx2" access="private" xsi:type="array" type="pointRs" size="1">
      <Value key="0" fatherId="fFrame1[0]" />
    </Data>
  </Datas>
</Database>
...
<Programs xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://www.staubli.com/robotics/VAL3/Program/2">
  ...
  movec(pPointRx,pPointRx2,tCurrentTool,mCurrentSpeed)
end]]></Code>
  </Program>
</Programs>
'''
...
class RobotPost(object):
    """Robot post object"""    
    #----------------------------------------------------------------------------
    # Set the IO board name as required by the dio data type. For official Staubli boards it should be BasicIO-1 or BasicIO-2
    IO_BOARD_NAME = "BasicIO-1"
    
    # Set the maximum number of lines per program
    PROG_MOVE_COUNT_MAX = 2000
    
    #----------------------------------------------------------------
    
    PROGRAM_DTX_ok = PROGRAM_DTX.replace("BasicIO-1", IO_BOARD_NAME)
    ...
    def ProgStart(self, progname):
        # 15 characters maximum (allow 99 subprograms)
        self.PROG_NAME = FilterName(progname[:13])
    ...
    def ProgSave(self, folder, progname, ask_user = False, show_result = False):
        ...
        if nprogs > 1:
            ...
            fid.write(self.PROGRAM_DTX_ok % (pose_2_str_dtx(self.REF), pose_2_str_dtx(self.TOOL)))
            fid.close()
            #-----------------------------------
            # stop.pgx
        ...
        for i in range(nprogs):
            ...
            #-----------------------------------
            # program.PJX
            project_file = folderprog_final + '/%s.pjx' % self.PROG_NAME_LIST[i] #self.PROG_NAME
            #show_file_list.append(project_file)
            fid = open(project_file, "w")
            fid.write(PROGRAM_PJX % (added_modules, self.PROG_NAME_LIST[i]))#self.PROG_NAME))
            fid.close()
            print('SAVED: %s\n' % project_file)
            #-----------------------------------
            # program.dtx
            program_data = folderprog_final + '/%s.dtx' % self.PROG_NAME_LIST[i] #self.PROG_NAME
    ...
    def waitDI(self, io_var, io_value, timeout_ms=-1):
        ...
        # at this point, io_var and io_value must be string values
        if timeout_ms < 0:
            self.addline('watch(%s==%s)' % (io_var, io_value))
        else:
            self.addline('watch(%s==%s,%.1f)' % (io_var, io_value, timeout_ms))
